...
public: 
    string showProcessRR(){
        ...
        // for (int i = 0; RR.size() != 0; i = i + quantum){
        //         Process need = RR.front(); 
        //         //cout << i << endl; 
        //         result = result + to_string(i) + characterLink + need.getNameProcess() + characterLink; 
        //         cout << result << endl; 
                
        //         // cap nhat thoi gian da chay cua Process 
        //         // va de kiem duyet xem co nen them vao hang doi tiep hay khong 
        //         for (int j = 0; i < _processes.size(); j++){
        //             // str2.compare(str2) == 0 => true 
        //             // != 0 => false
        //             //cout << j << endl; 
        //             if (need.getNameProcess().compare(_processes[j].getNameProcess()) == 0){
        //                 _processes[j].setTimeReal(i + quantum); 
        //                 cout << _processes[j].getNameProcess() << ": " << _processes[j].getTimeReal() << endl; 
        //                 if (_processes[j].getTimeReal() < _processes[j].getBurstTime()){
        //                     RR.push(_processes[j]); 
        //                 }
        //             }
...
public: 
    ...
    void writeDataToFileFCFS(){
        ...
        averageTotalTimeFCFS = sumTotalTimeFCFS / _processes.size(); 
        averageWaittingTimeFCFS = sumWaittingTimeFCFS / _processes.size(); 
    ...
    void writeDataToFileRR(){
        ...
        averageTotalTimeRR = sumTotalTimeRR / _processes.size(); 
        averageWaittingTimeRR = sumWaittingTimeRR / _processes.size(); 
...
int main (){
    ...
    //cout << "Quantum: " << data.getQuantum() << endl; 
    //data.showData();  
